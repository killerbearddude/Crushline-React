...
When the director says this, the assistant must use the GitHub connector to inspect the repository, determine the current task, execute the appropriate process step, and prepare a reviewable PR when repo changes are needed.
...
4. Confirm the task is `Approved`.
...
For implementation tasks, the assistant may prepare an execution branch/PR only if the required tools are available and the task can be completed safely.
